...
async function data_save(std_time, info_list) {
    for (let info of info_list) {
        ...
        global.data_ts[name][std_time].push(info["最新价"]-global.val_fix);
    }
...
async function data_calc() {
    // 计算 1,2,3,4,5,10,15,30,60 最大值,最小值,波动量
    let data_min = [1,2,3,4,5];
    let data_map = {}
...
async function data_alarm_cost(config, qh_pos) {
    let key = config.key;
...
async function data_alarm_pos(config, qh_calc) {
    // '白银': {
    //     '1': { max: 7853, min: 7852, lst: 7852, chg: 1 },
    //     '2': { max: 7859, min: 7852, lst: 7852, chg: 7 },
    //     '3': { max: 7865, min: 7852, lst: 7852, chg: 13 },
    //     '4': { max: 7871, min: 7852, lst: 7852, chg: 19 },
    //     '5': { max: 7877, min: 7852, lst: 7852, chg: 25 }
    // }

    // 判断区间内: 1.多空, 2.是否持续上涨/下跌, 3.区间内变化量
    // 1,2,3,4,5,10,15,30,60 最大值,最小值,波动量
...
async function price(config, std_time) {
    ...
    // TODO: 发送告警
    await data_alarm_pos(config.push_deer, qh_calc);

    // TODO: 发送告警
    // await data_alarm_cost(config.push_deer, qh_pos);
