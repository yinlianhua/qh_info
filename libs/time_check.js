...
function time_check (std_time, type="qh") {
    // 国内期货交易时间
    ...
    if (day >= 1 && day <= 5) {
        if (type == "qh") {
            // 期货
            // 时间段1
            if (now >= st1 && now <= et1) { res = true; }
            // 时间段2
            if (now >= st2 && now <= et2) { res = true; }
            // 时间段3
            if (now >= st3 && now <= et3) { res = true; }
            // 时间段4
            // if (now >= st4 && now <= et4) { res = true; } // 暂停夜盘
        } else {
            // 股票,基金
            // 时间段1
    ...
    if (res == false) {
        console.log(`不在交易时间内 ${moment().format("YYYY-MM-DD HH:mm:ss")}`);
    }
